...
if (Get-Command 'oh-my-posh' -ErrorAction Ignore && Test-Path "$HOME/.omp/themes/tupl.omp.json") {
    oh-my-posh init pwsh --config "$HOME/.omp/themes/tupl.omp.json" | Invoke-Expression
}
...
if (Get-Command 'terraform' -ErrorAction Ignore) { Set-Alias 'tf' 'terraform' }
...
# $env:PROTON_PASS_TOKEN = 'PROTON_PASS_TOKEN'
